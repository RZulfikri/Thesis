...
private let previewLog = OSLog(subsystem: Bundle.main.bundleIdentifier ?? "TrueDepthStreamer", category: "PreviewMetalView")
...
class PreviewMetalView: MTKView {
    ...
    /// Returns texture-space point normalized to [0, 1] range, suitable for focus/exposure APIs.
    func normalizedTexturePointForView(point: CGPoint) -> CGPoint? {
        guard let tp = texturePointForView(point: point), textureWidth > 0, textureHeight > 0 else { return nil }
        return CGPoint(x: tp.x / CGFloat(textureWidth), y: tp.y / CGFloat(textureHeight))
    }
    ...
    private func setupTransform(width: Int, height: Int, mirroring: Bool, rotation: Rotation) {
        ...
        // Resize aspect - use max to fill screen (crop edges if needed)
        ...
        // Remove centering - fill edge-to-edge instead
    ...
    override func draw(_ rect: CGRect) {
        ...
        guard let cvTexture = cvTextureOut, let texture = CVMetalTextureGetTexture(cvTexture) else {
            os_log(.error, log: previewLog, "Failed to create preview texture")
        ...
        guard let commandQueue = commandQueue else {
            os_log(.error, log: previewLog, "Failed to create Metal command queue")
            CVMetalTextureCacheFlush(textureCache!, 0)
            return
        }

        guard let commandBuffer = commandQueue.makeCommandBuffer() else {
            os_log(.error, log: previewLog, "Failed to create Metal command buffer")
            CVMetalTextureCacheFlush(textureCache!, 0)
            return
        }

        guard let commandEncoder = commandBuffer.makeRenderCommandEncoder(descriptor: currentRenderPassDescriptor) else {
            os_log(.error, log: previewLog, "Failed to create Metal command encoder")
            CVMetalTextureCacheFlush(textureCache!, 0)
            return
        }
