...
class DepthToJETConverter: FilterRenderer {
    ...
    private var minDepthMeters: Float = 0.10
    private var maxDepthMeters: Float = 0.60
    ...
    func prepare(with formatDescription: CMFormatDescription, outputRetainedBufferCountHint: Int) {
        ...
        if inputMediaSubType == kCVPixelFormatType_DepthFloat16 {
            ...
            assertionFailure("Unsupported depth input format: \(inputMediaSubType)")
        }
    ...
    /// Updates the depth range used for JET colormap rendering.
    /// Call this each frame with the detected closest-object depth to keep the colormap focused on the object.
    func updateDepthRange(min: Float, max: Float) {
        minDepthMeters = min
        maxDepthMeters = max
    }
    ...
    func render(pixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
        ...
        guard let outputPixelBuffer = newPixelBuffer else {
            assertionFailure("Allocation failure: Could not get pixel buffer from pool")
            return nil
        }
        
        let hist = histogramBuffer.contents().bindMemory(to: Float.self, capacity: Int(self.jetParams.histogramSize))
        
        // Apple Documentation: ARFrame.capturedDepthData provides depth values in METERS
        // Reference: https://developer.apple.com/documentation/arkit/arframe/2923543-captureddepthdata
        // TrueDepth Camera Working Range: ~10-60 cm (0.1-0.6 meters)
