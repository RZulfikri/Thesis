...
+(void) calcHistogramForPixelBuffer:(CVPixelBufferRef)pixelBuffer
    ...
    BOOL isFloat32 = (fmt == kCVPixelFormatType_DepthFloat32);
    ...
    for (size_t y = 0; y < height; ++y) {
        const uint8_t* row = baseAddress + y * stride;
        for (size_t x = 0; x < width; ++x) {
            float depth = isFloat32
                ? ((const float*)row)[x]
                : (float)((const __fp16*)row)[x];

            if (!isnan(depth) && depth > minDepth && depth < maxDepth) {
                int binIndex = (int)(depth * factor);
                if (binIndex >= 0 && binIndex < size) {
                    ++histogram[binIndex];
                    ++numPoints;
                }
            }
